...
- 접근제어자 분류
    ...
    - protected : 같은 패키지 내 또는 다른 패키지의 자식 클래스내에서만 접근 가능
...
- 캡슐화를 제대로 하기위한 코딩방법
    ...
    - Getter/Setter 메서드의 접근제어를 public, 
...
- 실행중 발생하는 오류 : 예외/런타임 에러. 수정이 어려움. 언제 발생할지 모름
    ...
    try {
        ...
        예외발생 유무와 상관없이 항상 처리할 코드 블럭;
    }
    ```

- 예외클래스 : ArithmeticException, ArrayIndexOutOfBoundsException,
...
- 예외를 직접 처리하지 않고 상위 클래스나 메서드에게 처리를 인가
    ...
    ```java
    리턴값 호출 메서드명(인자...) throws Exception {
        예외가 발생할 코드블럭;
        throw new Exception();
    }
...
#### 스트림 API
- 함수형 인터페이스로 데이터를 추상화하고 처리하는데 자주 사용되는 함수를 정의해 놓은 것
- 특징
    - 원본 데이터를 변경안함
    - 일회성
    - 내부 반복으로 작업처리
...
- 리스트와 유사, 문자열 처리때문에 메모리 문제, 성능문제가 발생하면 StringBuilder, StringBuffer 사용고려
...
- 입출력 예외가 발생할 수 있기 때문에 클래스나 메서드에 throws IOException을 추가해야 함
...
- 클래스명, 메서드 바로 위에 작성, 코드와 설정을 관리할 수 있게 도와주는 역할
